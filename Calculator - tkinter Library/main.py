...
# btn_click function updates the input field whenever a number is entered. Any button pressed will act as a button click update:
def btn_click(item):
    global expression
...
# btn_clear function clears the input field and previous calculations using the ’Clear’ button:
def btn_clear():
    global expression
    expression = ""
    input_text.set("")
...
# btn_equal function calculates the expression entered in the input field.
def btn_equal():
    global expression
...
# First row includes 2 buttons, ‘Clear (Clear)’ and ‘Divide (/)’:
...
# Second row includes 4 buttons, ‘7’, ‘8’, ‘9’ and ‘Multiply (*)’:
...
# Third row includes 4 buttons,  ‘4’, ‘5’, ‘6’ and ‘Subtract (-)’:
...
# Fourth row includes 4 buttons, ’1’, ’2’, ’3’, and ’plus (+)’:
...
# Fifth row includes 3 buttons,  ‘0’, ‘Decimal (.)’, and ‘Equal To (=)’:
